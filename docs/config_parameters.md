...
- **Описание**:
    ...
    Если задано значение `0`, то вы не увидите меню Clover, выбранная по умолчанию для загрузки система сразу будет загружаться. Однако, если нажать пробел - отобразится меню Clover. Этот параметр с этим значением аналогичен параметру [Fast](#22-fast) со значением `true`.
...
- **Описание**:
    ...
    С включением этого параметра система, начнет запускаться с раздела, заданного в параметре [DefaultVolume](32-defaultvolume) сразу после запуска компьютера.
